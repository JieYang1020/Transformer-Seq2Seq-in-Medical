...
def deal_label(label_data):
    labels_one_hot = np.zeros(label_data.size(0))
    for idx2, label_one_hot in enumerate(labels_one_hot):
        if label_data[idx2] != 0:
            labels_one_hot[int(label_data[idx2].item()) - 1] = 1
...
def train(train_data_loader, val_data_loader):
    ...
    for epoch in range(config.num_epochs):
        ...
        test_loss = 0; TP = TN = FN = FP = 0
        for batch_idx, (content, label) in enumerate(val_data_loader):
            label = label.to(torch.float32)
            predicted = model(content)
            for sentence_idx, update_real_label in enumerate(label):
                #标签调整
                update_real_label = deal_label(update_real_label)
                update_predicted_label = torch.gt(F.sigmoid(predicted[sentence_idx]), 0.9)
                update_real_label = torch.gt(update_real_label, 0.5)
                TP += ((update_predicted_label == 1)&(update_real_label == 1)).sum()
                TN += ((update_predicted_label == 0)&(update_real_label == 0)).sum()
                FN += ((update_predicted_label == 0)&(update_real_label == 1)).sum()
                FP += ((update_predicted_label == 1)&(update_real_label == 0)).sum()
            precision = (TP.item()/(TP+FP).item())/label.size(0)
            recall = (TP.item() / (TP + FN).item()) / label.size(0)
...
if __name__=="__main__":
    ...
    val_data_loader = DataLoader(dataset=val_db,batch_size=config.batch_size,shuffle=True,num_workers=0)
    train(train_data_loader, val_data_loader)
    # for idx, (data, label) in enumerate(train_data_loader):
    #     for sentence_idx, sentence in enumerate(label):
